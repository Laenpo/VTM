...
final class NLLBTranslator: ObservableObject, @unchecked Sendable, TranslationEngine {
    ...
    init() {
        ...
        // Whisper 即将加载 → NLLB 主动卸载让出内存
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleWhisperWillLoad),
            name: NSNotification.Name("WhisperWillLoad"),
            object: nil
        )
    }
    ...
    @objc private func handleWhisperWillLoad() {
        VTMLog.memory("📢 收到 WhisperWillLoad 通知 → 卸载 NLLB 释放内存")
        unload()
    }
    ...
    func loadModel(
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if self.encoderSession != nil && self.decoderSession != nil {
                ...
                VTMLog.error("⚠️ NLLB session 在加载后被清除，需要重新加载", category: "Model")
            }
